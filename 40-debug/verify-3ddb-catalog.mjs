...
assert(!high3ddb.panelOpen, "3DDB panel should default to collapsed");
assert(!high3ddb.mapLayersVisible, "map layers hidden when panel collapsed");

await page.evaluate(() => window.__gaikuViewerTest.set3ddbPanelOpen(true));
await page.waitForTimeout(300);

const open3ddb = await page.evaluate(() => window.__gaikuViewerTest.get3ddbUi());
console.log("panel open:", open3ddb);
assert(open3ddb.panelOpen, "3DDB panel should open");
assert(open3ddb.mapLayersVisible, "map layers visible when panel open and toggle on");

await page.evaluate(() => window.__gaikuViewerTest.set3ddbPanelOpen(false));
await page.waitForTimeout(300);

const closed3ddb = await page.evaluate(() => window.__gaikuViewerTest.get3ddbUi());
assert(!closed3ddb.panelOpen, "3DDB panel should close");
assert(!closed3ddb.mapLayersVisible, "map layers hidden when panel collapsed again");
