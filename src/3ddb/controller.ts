...
export class DddbCatalogController {
  ...
  constructor(
    ...
    this.ui.applyDisclaimer(config);
  ...
  private syncMapLayers(): void {
    const { panelOpen, toggleChecked } = this.ui.getUiState();
    this.layers.setLayersVisible(this.active && panelOpen && toggleChecked);
  }
  ...
  getUiState(): DddbCatalogUiState {
    return {
      ...this.ui.getUiState(),
      mapLayersVisible: this.layers.isLayersVisible(),
    };
  }
}
